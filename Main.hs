...
eval env (SymbolDefE name expr) =
  case (eval env expr) of
    (env', val) -> (insert name val env', NilRes)

eval env (DoE es) =
  Prelude.foldl (\ (env', _) e -> eval env' e) (env,NilRes) es
...
eval env (FuncInvoke name argEs) =
  case Data.Map.lookup name env of
    Just (FunctionR fenv argNames body) ->
      ...
      where
        newEnv = union (fromList
                        (zip argNames
                         (Prelude.map (\e -> snd $ eval env e) argEs))) fenv
    _ -> (env, Error)
...
eval env (Add e1 e2) =
  case ((eval env e1), (eval env e2)) of
    ((env1, IntRes i1), (env2, IntRes i2)) -> (union env1 env2, IntRes (i1 + i2))
    _ -> (env, Error)
...
eval env (Divide e1 e2) =
  case ((eval env e1), (eval env e2)) of
    ((env1, IntRes i1), (env2, IntRes i2)) -> (union env1 env2, IntRes (quot i1 i2))
    _ -> (env, Error)

eval env (And e1 e2) =
  case (eval env e1, eval env e2) of
    ((env1, BoolRes t1), (env2, BoolRes t2)) -> (union env1 env2, BoolRes (and [t1, t2]))
    _ -> (env, Error)

eval env (Or e1 e2) =
  case (eval env e1, eval env e2) of
    ((env1, BoolRes t1), (env2, BoolRes t2)) -> (union env1 env2, BoolRes (or [t1, t2]))
    _ -> (env, Error)
